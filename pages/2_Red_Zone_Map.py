...
render_page_header(
    "Red Zone Map",
    "Spatial view of demonstration hazard zones and habitation risk classes. Replace demo layers with authoritative GIS products during integration.",
)
...
with st.sidebar:
    ...
    show_hazards = st.checkbox("Show hazard polygons", value=True)
...
if show_hazards and hazards is not None:
    ...
    folium.GeoJson(
        hazards.__geo_interface__,
        name="Demonstration Hazard Zones",
        style_function=hazard_style,
        tooltip=folium.GeoJsonTooltip(
            fields=["hazard_type", "hazard_score", "data_mode"],
            aliases=["Hazard", "Hazard Score", "Data Mode"],
            sticky=False,
        ),
    ).add_to(map_obj)
...
for row in filtered.to_dict(orient="records"):
    ...
    popup = (
        ...
        f"Priority: {row['relocation_priority']}<br>"
        f"Hazard: {row.get('hazard_type') or 'Demo score'}"
    )
    folium.CircleMarker(
...
with left:
    ...
    if hazards is None:
        st.warning("Hazard polygon layer is unavailable; stored demo hazard scores are being used instead.")
    else:
        st.caption(
            "Hazard polygons and habitation points on this page are explicitly synthetic demonstration data for end-to-end GIS testing."
        )
...
with right:
    ...
    if selected.get("inside_hazard_zone") is True:
        st.error("Location intersects a demonstration hazard polygon.")
    elif selected.get("inside_hazard_zone") is False:
        st.info(f"Nearest hazard boundary: approximately {selected.get('distance_to_hazard_km', 0):.2f} km")
    st.markdown("**Coordinates**")
    st.code(f"{selected['latitude']:.5f}, {selected['longitude']:.5f}")

st.markdown(
    "**Risk legend:** CRITICAL = red · HIGH = orange · MODERATE = amber · LOW = green"
)
render_disclaimer()
